...
def get_subsample(
    ...
    is_outlier = np.sum(
        np.logical_or(dataset >= max_accepted_value, dataset <= min_accepted_value),
        axis=1,
    )
...
def initial_population(
    ...
    gene_array = np.reshape(gene_array.T, [nb_init_individuals, gates_per_qubits, nb_qubits, 5]).reshape(
...
def get_gene_space(gate_dict: dict, nb_features: int, nb_qubits: int, gates_per_qubits: int) -> list[int]:
...
def gen_int(min_val: int, max_val: int, size: int = None, exclude_array: np.ndarray = None):
    ...
    for j in range(gates_per_qubits):
        for k in range(nb_qubits):
            ...
            if gate_list[gate_type_idx] in gate_dict["single_non_parametric"]:
                ...
                if k < 3:
                    if [physical_qubits[k], physical_qubits[k + 1]] in coupling_map or (
                        physical_qubits[k],
                        physical_qubits[k + 1],
                    ) in coupling_map:
                        gate(k, k + 1)
                ...
                if multi_features == 1 and feature_transformation_type == 1:
                    if second_feature_idx not in x_idxs:
                        x_idxs.append(second_feature_idx)
                    param_expression = (
                        (2 * np.pi * x[first_feature_idx] * (1 - x[second_feature_idx]) - np.pi)
                        * (2 * np.pi * x[second_feature_idx] * (1 - x[first_feature_idx]) - np.pi)
                        / np.pi
                    )
...
def fitness_func_wrapper(
    ...
    def fitness_func(
    ...
    ) -> Union[np.float64, list[np.float64]]:
        ...
        init_solutions = pd.read_csv(
            save_path + "/last_generation_genes_" + suffix + ".csv",
            header=None,
            index_col=False,
        ).to_numpy()
        fitness_value = pd.read_csv(
            save_path + "/last_generation_fitness_values_" + suffix + ".csv",
            header=None,
            index_col=False,
        ).to_numpy()
        ...
        with open(
            save_path + "/fitness_values_iter_double_check_" + suffix + ".txt",
            "a",
            encoding="UTF-8",
        ) as file:
        ...
        for j in range(i):
            ...
            for q in range(fmap.num_qubits):
                ...
                exp_term = exp_term + np.linalg.norm(
                    partial_trace(statevector_i_dm, summed_qubits) - partial_trace(statevector_j_dm, summed_qubits)
                )
...
def parallel_qpu_run_wrapper(
    ...
    def parallel_qpu_run(ga_instance, offspring_mutation):
        ...
        compute_parallel_kernel_save_data(
            data_cv,
            data_labels,
            backend,
            gate_dict,
...
def compute_parallel_kernel_save_data(
    ...
    flattened_qsvm_connections = [item for sublist in qsvm_connections for item in sublist]
    ...
    for k, child in enumerate(genes):
        fmap, x_idxs = to_quantum(
            ...
            nb_qubits,
            0,
            qsvm_connections[shuffle_index[k]],
    ...
    if n_gen != 0 or recovery_mode == False:
        for k in range(len(genes)):
            with open(
                save_path + "/last_generation_genes_mutated" + suffix + ".csv",
                "w" if k == 0 else "a",
                encoding="UTF-8",
            ) as file:
        ...
        for i in range(nb_samples):
            ...
            for j in range(i + 1, data_cv.shape[0]):
                ...
                for k, rand_idx in enumerate(shuffle_index):
                    ...
                    bound_circuit = (
                        quantum_circuit_list[k]
                        .assign_parameters(data_cv[i, x_idxss[k]])
                        .compose(quantum_circuit_list[k].assign_parameters(data_cv[j, x_idxss[k]]).inverse())
                    )
                    # bound_circuit = transpile(bound_circuit, basis_gates=basis_gates, coupling_map= coupling_map_model)
                    combined_circuit.compose(
                        bound_circuit,
                        qubits=[qsvm_connections[rand_idx][l] for l in range(nb_qubits)],
                        inplace=True,
                    )
                ...
                #     combined_circuit)
                if (
                    counter % max_circuit_per_job == max_circuit_per_job - 1
                    or counter == (nb_samples**2 - nb_samples) / 2 - 1
                ):
        ...
        with Session(backend=backend):  # , service=service):
            job_executions = [
                backend.run(combined_circuits[i], shots=8000) for i in range(counter // max_circuit_per_job + 1)
            ]
    ...
    for i in range(nb_samples):
        for j in range(i + 1, data_cv.shape[0]):
            ...
            counter += 1
            for k, rand_idx in enumerate(shuffle_index):
                generation_kernels[i, j, k] = get_kernel_entry(
                    cbits[nb_qubits * rand_idx : nb_qubits * (rand_idx + 1)],
                    counts,
                    nb_qubits,
                )
    ...
    for k in range(len(genes)):
        generation_kernels[:, :, k] += generation_kernels[:, :, k].T + np.eye(nb_samples)
        ...
        Path("../../Output_genetic").mkdir(exist_ok=True)
        Path(save_path).mkdir(exist_ok=True)
        with open(save_path + "/genes" + suffix + ".csv", "a", encoding="UTF-8") as file:
            writer = csv.writer(file)
            writer.writerow(genes[k])
        with open(save_path + "/kernels_flattened" + suffix + ".csv", "a", encoding="UTF-8") as file:
            writer = csv.writer(file)
            writer.writerow(generation_kernels[:, :, k].reshape(-1))
        with open(save_path + "/depth" + suffix + ".txt", "a", encoding="UTF-8") as file:
            file.write(str(fmap_transpiled_depth) + "\n")
        with open(save_path + "/sparsity" + suffix + ".txt", "a", encoding="UTF-8") as file:
            file.write(str(sparsity_cost) + "\n")
        with open(save_path + "/accuracy" + suffix + ".txt", "a", encoding="UTF-8") as file:
            file.write(str(accuracy_cv_cost) + "\n")
        with open(save_path + "/fitness_values_iter_" + suffix + ".txt", "a", encoding="UTF-8") as file:
            file.write(str(fitness_value) + "\n")
        with open(save_path + "/offdiagonal_mean_" + suffix + ".txt", "a", encoding="UTF-8") as file:
            file.write(str(offdiagonal_mean) + "\n")
        with open(save_path + "/offdiagonal_std_" + suffix + ".txt", "a", encoding="UTF-8") as file:
        ...
        with open(
            ...
            writer = csv.writer(file)
            writer.writerow(genes[k])
    with open(save_path + "/rand_indices" + suffix + ".csv", "a", encoding="UTF-8") as file:
        writer = csv.writer(file)
